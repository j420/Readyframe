...
def _rubric_hash(version: str) -> str:
    """Hash loaded rubric content locally to tolerate serverless module bundle skew."""
    return _canonical_hash(load_rubric(version))
...
def _assemble(sub_scores: list[dict], rubric_version: str, inputs: dict, drivers: list[dict], confidence: float, dimensions=None, bands=BANDS) -> dict:
    ...
    audit = {"rubric_version": rubric_version, "inputs_hash": _canonical_hash(inputs), "rubric_hash": _rubric_hash(rubric_version if rubric_version != "r1" else "2026.07.0"), "engine_version": ENGINE_VERSION, "generated_at": "1970-01-01T00:00:00Z"}
