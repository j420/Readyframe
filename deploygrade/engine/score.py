...
BANDS = ((400, "BLOCKED"), (600, "CONDITIONAL"), (800, "READY"))
DIMENSIONS = (
    ("change_management", .18, ("ci", "iac"), ("PCI DSS 6.4.5.4", "SOC 2 CC8.1", "NIST SP 800-53 CM-3"), 3, "Add a reviewed change-control gate"),
    ("privileged_access", .17, ("iam",), ("HIPAA 164.312", "NIST SP 800-53 CM-5"), 4, "Enforce least-privilege IAM controls"),
    ("rollback_recovery", .18, ("rollback",), ("NIST SP 800-53 CM-5",), 4, "Add an automated rollback path"),
    ("verification", .16, ("tests",), ("PCI DSS 6.4.5.4",), 2, "Add meaningful automated verification"),
    ("observability", .15, ("cloud",), ("SOC 2 CC8.1",), 3, "Add production observability evidence"),
    ("evidence_governance", .16, (), ("HIPAA 164.312", "SOC 2 CC8.1"), 2, "Record governed evidence for required controls"),
)
...
def _next_threshold(score: int) -> int | None:
    return next((threshold for threshold, _ in BANDS if score < threshold), None)


def _counterfactual(sub_scores: list[dict], score: int) -> list[dict]:
    target = _next_threshold(score)
    if target is None:
        return []
...
def score_inventory(inventory: dict, rubric_version: str = "2026.07.0") -> dict:
    ...
    return _assemble(sub_scores, rubric_version, inventory, drivers, min(evidence_confidences))


def _assemble(sub_scores: list[dict], rubric_version: str, inputs: dict, drivers: list[dict], confidence: float) -> dict:
    ...
    result = {"$schema": "../schemas/readiness_score.schema.json", "schema_version": "2.0",
              ...
              "band": band_for(value), "sub_scores": sub_scores,
              ...
              "counterfactual": _counterfactual(sub_scores, value), "audit": audit}
